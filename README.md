...
3D는 실행을 원자 구조로 보여준다. 장식이 되지 않도록 모든 요소가 정보를 나른다.

| 원자 | 실행 그래프 |
|---|---|
| 핵 | 루트 에이전트 |
| 양성자 | 생성된 하위 에이전트 수 |
| 중성자 | 도구 호출 수 (→ 실행될수록 핵이 자란다) |
| 전자 | 하위 에이전트 (상태 = 색) |
| 위성 | 그 에이전트의 도구 호출 |
| 스포크 | 부모-자식 연결 |
| 에너지선 | 에이전트 간 메시지 |

계층은 좌표가 아니라 **"중심에서 얼마나 뻗어 나갔는가"**로 표현된다.
그래서 2D와 달리 ELK 좌표를 쓰지 않는다.

**위치는 정적이다.** 물체가 각자 공전하면 눈이 쉴 곳이 없어 오히려 안 읽힌다.
움직이는 것은 둘뿐이고 각각 이유가 있다 — 카메라의 느린 자동 회전(끌 수 있다)과,
지금 무엇이 오가는지 알려주는 메시지 스파크.

배치는 피보나치 나선으로 방향을 흩고 세로를 눌러(FLATTEN) 공이 아닌 방사형으로
퍼지게 한다. 도구는 자기 에이전트에서 **바깥쪽으로** 부챗살을 편다 — 그래서 안쪽
물체와 겹치지 않는다 (검증: 최소 간격 105px, 구 지름 36px).
...
- 노드 하나가 그 자체로 **원자**다 (`AtomOrb.tsx`). 매끈한 구는 당구공처럼 보인다.
  여섯 겹을 겹쳐 홀로그램으로 만든다 — 코어 / 안개 / 침 / 지오데식 껍질 / 먼지 / 고리.
  모든 겹이 가산 혼합이라 겹칠수록 밝아진다. 이게 빛으로 읽히는 핵심.
- 구체 — 발광 재질(`emissiveIntensity`) + `Bloom`, 실행 중인 것만 맥동
- 스포크·에너지선 — `Line`. 위치가 고정이라 한 번만 만든다
