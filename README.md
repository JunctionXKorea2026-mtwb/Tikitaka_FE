...
`Tikitaka` 와 `START` 만 있는 랜딩. 가운데에 **원자 하나**가 떠 있다
(`features/landing/`, 측지선 격자 detail 3 — 꼭짓점 642, 모서리 1920).

START를 누르면 그 원자가 급격히 퍼지면서 화면을 삼키고 본 화면으로 넘어간다.
**커지는 동시에 옅어진다** — 커지기만 하면 벽에 부딪히는 것처럼 보이고,
옅어져야 "구조 안으로 들어간다"가 된다. 별도 함께 밀려나 지나가는 느낌을 만든다.
가속 곡선은 `t³` 이라 처음엔 느리고 끝에 빨려든다.

글자는 원자보다 **먼저 빠진다** (0.34초). 퍼지는 동안 시야를 비워 주기 위해서다.
진행도는 ref 하나로 공유한다 — 매 프레임 state를 바꾸면 React가 따라오지 못한다.

글자와 버튼은 DOM으로 즉시 그리고 3D 배경만 나중에 붙는다. three는 무거워서
기다렸다 함께 띄우면 첫 화면이 한참 비어 있게 된다. WebGL이 없는 환경에서도
글자와 버튼은 그대로 보인다.

`started`는 저장하지 않는다 — 새로 들어오면 첫 화면을 다시 보는 게 자연스럽다.
다만 **이미 나눈 대화가 있으면 건너뛴다.** 이어서 보러 온 사람에게는 방해다.
